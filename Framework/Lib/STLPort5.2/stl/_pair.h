...
struct pair {
  ...
  __TRIVIAL_DESTRUCTOR(pair)
    ...
    // SGE: provide no-op assignment operator to ensure that we can at least
    // compile specialized hashtable, etc. (See slist::operator=)
    pair & operator=( const pair<_T1,_T2> ){ return *this; }
};
