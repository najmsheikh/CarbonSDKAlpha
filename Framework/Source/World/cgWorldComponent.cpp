...
//-----------------------------------------------------------------------------
// cgWorldComponent Module Includes
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Name : cgWorldComponent() (Constructor)
/// <summary>Object class constructor.</summary>
//-----------------------------------------------------------------------------
cgWorldComponent::cgWorldComponent( cgUInt32 nReferenceId, cgWorld * pWorld ) : cgReference( nReferenceId )
{
...
/// <summary>Object class constructor.</summary>
//-----------------------------------------------------------------------------
cgWorldComponent::cgWorldComponent( cgUInt32 nReferenceId, cgWorld * pWorld, cgWorldComponent * pInit ) : cgReference( nReferenceId )
{
...
//-----------------------------------------------------------------------------
// Name : ~cgWorldComponent() (Destructor)
/// <summary>Clean up any resources being used.</summary>
//-----------------------------------------------------------------------------
cgWorldComponent::~cgWorldComponent()
{
    // Clean up
    dispose( false );
}
...
//-----------------------------------------------------------------------------
void cgWorldComponent::dispose( bool bDisposeBase )
{
...
/// </summary>
//-----------------------------------------------------------------------------
bool cgWorldComponent::createTypeTables( const cgUID & TypeIdentifier )
{
...
/// </summary>
//-----------------------------------------------------------------------------
void cgWorldComponent::onComponentModified( cgComponentModifiedEventArgs * e )
{
    cgMessage                   Msg;
...
/// </summary>
//-----------------------------------------------------------------------------
bool cgWorldComponent::onComponentCreated( cgComponentCreatedEventArgs * e )
{
    cgMessage                   Msg;
    EventListenerList::iterator itListener;
...
/// </summary>
//-----------------------------------------------------------------------------
bool cgWorldComponent::onComponentLoading( cgComponentLoadingEventArgs * e )
{
    cgMessage                   Msg;
    EventListenerList::iterator itListener;
    ...
    if ( mReferenceId == e->sourceRefId )
    {
        ...
        {
            cgString strSQL = cgString::format( _T("SELECT RefCount FROM '%s' WHERE RefId=%i"), this->getDatabaseTable().c_str(), mReferenceId );
...
/// </summary>
//-----------------------------------------------------------------------------
void cgWorldComponent::onComponentDeleted( )
{
    cgMessage                   Msg;
    EventListenerList::iterator itListener;
...
/// </summary>
//-----------------------------------------------------------------------------
void cgWorldComponent::onReferenceAdded( cgReference * pHolder, cgInt32 nHolderReferences, bool bReconnecting )
{
...
/// </summary>
//-----------------------------------------------------------------------------
void cgWorldComponent::onReferenceRemoved( cgReference * pHolder, cgInt32 nHolderReferences, bool bDisconnecting )
{
...
/// </summary>
//-----------------------------------------------------------------------------
bool cgWorldComponent::queryReferenceType( const cgUID & type ) const
{
    // Supports this interface?
    if ( type == RTID_WorldComponent )
        return true;
...
/// </summary>
//-----------------------------------------------------------------------------
cgUInt32 cgWorldComponent::getLocalTypeId( ) const
{
...
/// </summary>
//-----------------------------------------------------------------------------
cgWorld * cgWorldComponent::getParentWorld( ) const
{
    return mWorld;
}
...
/// </summary>
//-----------------------------------------------------------------------------
bool cgWorldComponent::shouldSerialize( ) const
{
