...
class AntialiasingShader : ISurfaceShader
{
	...
	//-------------------------------------------------------------------------
	// Name : initializeFXAA()
	// Desc : Sets up the fxaa pre-processor values for shader compilation
	//-------------------------------------------------------------------------
    void initializeFXAA( int qualityLevel )
    {
		// Set up the fxaa quality preset
		switch ( qualityLevel )
		{
	...
	//-----------------------------------------------------------------------------
    bool fxaa( int qualityLevel, bool reverseToneMap )
    {
        /////////////////////////////////////////////
        // Definitions
        /////////////////////////////////////////////
		...
		// Run the fxaa initialization function
	    initializeFXAA( qualityLevel );
		...
		// is always expontial with an exposure of 0.01, and squared to approximate sRGB (i.e., y = ( 1.0f - exp( -color * 0.01 ) ) ^ 2)
		if ( reverseToneMap )
			<?color.rgb = -log( 1.0f - sqrt( color.rgb ) ) / 0.01;?> 
		
		// Optionally apply a vignette
