...
bool cGamePlayState::loadScene( )
{
    ...
    for ( cgInt i = 0; i < 10; ++i )
    {
        ...
        cgObjectNode * agent = mScene->loadObjectNode( 0x2B8, cgCloneMethod::ObjectInstance, true );
        
        // Enable navigation and set initial task somewhere in the distance.
        agent->setPosition( pos + cgVector3(0,0,-20.0f));
        agent->enableNavigation( &params );
        agent->navigateTo( pos + cgVector3( 0.0f, 0.0f, 40.0f) );
