...
//---------------------------------------------------------------------------//
//                                                                           //
// Name : Main.gs (7. Spawning Objects)                                      //
//                                                                           //
// Desc : Script containing the majority of the logic for demonstrating how  //
//        objects can be spawned into the scene procedurally at runtime.     //
//                                                                           //
//---------------------------------------------------------------------------//
...
class Main : IScriptedAppState
{
    ...
    bool begin( )
    {
        ...
        @mForm = interfaceManager.loadForm( "appdir://Scripts/7. Spawning Objects/SpawnObjects.frm", "spawningForm" );
        if ( @mForm == null )
            return false;
    ...
    void update( )
    {
        ...
        if ( @mScene != null && !mState.isSuspended() )
        {
            ...
            if ( inputDriver.isKeyPressed( Keys::LControl ) )
            {
            ...
            float cameraOffset = (controller.getCharacterHeight( true ) * 0.5f) * 0.95f;
    ...
    void render( )
    {
        ...
        if ( mSceneView.begin() )
        {
            ...
            interfaceManager.printText( rcScreen, "\n\n[c=#ffcccccc]<RMB>[/c] to pickup an object. [c=#ffcccccc]<LMB>[/c] to throw.\nPress [c=#ffff0000]<Tab>[/c] to show/hide spawn menu.", TextFlags::Multiline | TextFlags::AllowFormatCode, 0xFFFFFFFF, 0, 10 );
